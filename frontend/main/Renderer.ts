import type { Vector, BallState } from '../GameLogic/type.js';
...
export class Renderer {
    ...
    private aimCanvas: HTMLCanvasElement;
    private aimCtx: CanvasRenderingContext2D;

    // 球的颜色配置
    private readonly BALL_COLORS: { [key: number]: string } = {
        0: '#FFFFFF',   // 母球（白色）
        1: '#FFD700',   // 1号（黄色）
        2: '#0000FF',   // 2号（蓝色）
        3: '#FF0000',   // 3号（红色）
    ...
    constructor(canvasId: string, aimCanvasId: string) {
        this.canvas = document.getElementById(canvasId) as HTMLCanvasElement;
        this.ctx = this.canvas.getContext('2d')!;
        
        this.aimCanvas = document.getElementById(aimCanvasId) as HTMLCanvasElement;
     ...
     * 渲染游戏画面
     */
    public render(ballStates: BallState[]): void {
        this.clear();
        this.drawTable();
        this.drawPockets();
     ...
     * 绘制所有球
     */
    private drawBalls(ballStates: BallState[]): void {
     ...
     * 绘制瞄准线
     */
    public drawAimLine(cueBallPos: Vector, angle: number): void {
        this.clearAimLine();
